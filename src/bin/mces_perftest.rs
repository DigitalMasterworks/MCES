//! §10.1.0 MCES Peripheral Roundtrip (FULL AEAD, two-phase: capture → crypto)
//! Usage:
//!   cargo run --release --bin mces_perftest -- <device_path> [rounds=200] [chunk_mb=1]
//! Example:
//!   cargo run --release --bin mces_perftest -- /dev/video0 200 1
...
use mces::headers::{MCES_HEADER_BYTES, MCES_TAG_BYTES, VaultHeader, VAULT_VERSION};
...
fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        eprintln!(
            ...
            args.get(0).map(|s| s.as_str()).unwrap_or("mces_perftest")
        );
        std::process::exit(1);
    }
    let devpath = &args[1];
    ...
    println!(
        "[mces_perftest] vault_params: ver=0x{:02X} cps={} ts={} nonce={:02X?} k_stream_len={} rounds={} {}",
        VAULT_VERSION,
        pw.chars().count(),
        timestamp_ns,
        &nonce12,
    ...
    println!("[mces_perftest] password: {}", pw);
    ...
    if is_v4l {
        println!("[mces_perftest] capture: {:.2} MB in {:.3}s → {:.2} MB/s @ {:.2} FPS",
            cap_mb, cap_secs, cap_mbps, cap_fps
        );
    } else {
        println!("[mces_perftest] capture: {:.2} MB in {:.3}s → {:.2} MB/s",
    ...
    println!(
        "[mces_perftest] crypto: enc={:.3}s ({:.2} MB/s) • dec={:.3}s ({:.2} MB/s) • bytes={:.2} MB",
        enc_secs, enc_mbps, dec_secs, dec_mbps, mb
    );
    println!(
        "[mces_perftest] header={}B tag={}B (BLAKE3 keyed) • ver=0x{:02X}",
        MCES_HEADER_BYTES, MCES_TAG_BYTES, VAULT_VERSION
    );
