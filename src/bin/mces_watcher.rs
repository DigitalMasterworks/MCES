//! §7.1.0 Overview — MCES Watcher (separate process)
//! Watches folders listed in ~/.mces_watchfolders for new/changed files,
//! encrypts them with `mces encrypt`, parses the generated password, and
...
fn main() -> Result<()> {
    ...
    if folders.is_empty() {
        eprintln!("No folders to watch. Add some to ~/.mces_watchfolders and re-run. Exiting.");
        return Ok(());
    }

    eprintln!("[MCES Watcher] Watching {} folder(s):", folders.len());
    for f in &folders {
        eprintln!("  - {}", f.display());
    }
    ...
    for res in rx {
        let event = match res {
            Ok(ev) => ev,
            Err(e) => {
                eprintln!("[MCES Watcher] notify error: {e}");
                continue;
            }
        };
        ...
        for p in event.paths {
            ...
            thread::spawn(move || {
                if let Err(e) = handle_file_event(&p) {
                    eprintln!("[MCES Watcher] Error for {}: {e:#}", p.display());
                }
            });
        }
    }
...
fn handle_file_event(path: &Path) -> Result<()> {
    ...
    if !wait_for_stable(path, Duration::from_secs(1), Duration::from_millis(50))? {
        eprintln!("[MCES Watcher] File did not stabilize: {}", path.display());
        return Ok(());
    }

    eprintln!("[MCES Watcher] Encrypting: {}", path.display());

    // mces encrypt <file>
    let (password_opt, combined_out) = run_mces_encrypt(path)?;
    ...
    if let Some(pw) = password_opt {
        if vault.is_file() {
            ...
            if let Err(e) = sigilbook_save(&vault, &pw) {
                eprintln!(
                    "[MCES Watcher] Failed to save password to sigilbook: {} ({:#})",
                    vault.display(),
                    e
                );
            } else {
                eprintln!("[MCES Watcher] Password saved: {}", vault.display());
            }
        } else {
            eprintln!(
                "[MCES Watcher] Encrypt reported password, but vault not found: {}",
                vault.display()
            );
        ...
        eprintln!(
            "[MCES Watcher] Encrypt failed or no password captured: {}",
            path.display()
        );
        eprintln!("{}", combined_out);
    }
...
fn run_mces_encrypt(file: &Path) -> Result<(Option<String>, String)> {
    let output = Command::new("mces")
        .arg("encrypt")
        .arg(file)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
        .with_context(|| format!("launch mces encrypt for {}", file.display()))?;
