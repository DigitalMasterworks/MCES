...
// Decrypts in-process via MCES library; never calls mces_decrypt.
//
// Build: part of the existing Cargo workspace alongside mces & sigilbook.
//
// Behavior summary:
...
const PRELOAD_DIRNAME: &str = "velvet_preload"; // Can rename to "mces_preload" later if you wish
...
// --------- MCES decryption in-process (to RAM) ---------
...
fn decrypt_vault_to_ram(
    ...
    // Build MCES config + postmix and stream-decrypt into /dev/shm
    fin.seek(SeekFrom::Start(ct_start))?;
