// src/bin/mces_cam_gui.rs
// MCES SecureCam GUI — multi-cam + mic combos with muxed A/V .vaults
// Build: cargo run --release --bin mces_cam_gui
...
fn write_mux_record(
    ...
    mac.update(b"MCES2DU-MUX-FRAME-v1");
...
fn spawn_ffplay_video_mjpeg() -> Option<std::process::ChildStdin> {
    let args: Vec<String> = vec![
        ...
        "-f".into(), "mjpeg".into(),
        "-i".into(), "-".into(),
        "-window_title".into(), "MCES Video".into(),
    ];
...
fn spawn_ffplay_audio_48k2() -> Option<std::process::ChildStdin> {
    let args: Vec<String> = vec![
        ...
        "-ac".into(), "2".into(),
        "-i".into(), "-".into(),
        "-window_title".into(), "MCES Audio".into(),
    ];
    Command::new("ffplay").args(&args).stdin(Stdio::piped()).stdout(Stdio::null()).stderr(Stdio::null()).spawn().ok()?.stdin.take()
}
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native("MCES SecureCam", opts, Box::new(|_cc| Box::<App>::default()))
}