...
pub const SARX_CHUNK: usize = 4 << 20;      // 4 MiB
pub const SARX_HEADER_BYTES: usize = 61;    // fixed header size
pub const SARX_TAG_BYTES: usize = 32;       // BLAKE3 keyed tag size
...
impl VaultHeader {
    pub fn encode(&self) -> [u8; SARX_HEADER_BYTES] {
        let mut out = [0u8; SARX_HEADER_BYTES];
        out[0..4].copy_from_slice(b"SARX");
        out[4] = VAULT_VERSION;
    ...
    pub fn decode(buf: &[u8]) -> Result<Self> {
        if buf.len() < SARX_HEADER_BYTES { bail!("short header"); }
        if &buf[0..4] != b"SARX" { bail!("bad magic"); }
    ...
    pub fn decode_with_raw(buf: &[u8]) -> Result<(Self, [u8; SARX_HEADER_BYTES])> {
        if buf.len() < SARX_HEADER_BYTES { bail!("short header"); }
        let mut raw = [0u8; SARX_HEADER_BYTES];
        raw.copy_from_slice(&buf[0..SARX_HEADER_BYTES]);
