...
// ============================================================================
/* §1.10.1 encrypt_mces */
pub fn encrypt_mces(pt: &[u8], cfg: &MCESConfig, _postmix: Option<&[u8]>, ct: &mut [u8]) -> Result<()> {
...
pub fn thermo_harden_okm(okm: &mut [u8]) {
    ...
    {
        let mut h = Blake3::new();
        h.update(b"MCES2DU-THERMO-SEED-v1");
        h.update(okm);
    ...
    {
        let mut h = Blake3::new();
        h.update(b"MCES2DU-THERMO-SCRATCH-v1");
        h.update(&seed);
    ...
    {
        let mut h = Blake3::new();
        h.update(b"MCES2DU-THERMO-MIX-v1");
        let mut tmp = [0u8; 8];
