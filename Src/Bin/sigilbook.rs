...
//!
//! /* =============================================================================
//!  * MCES — sigilbook.rs — Program v6.0.0
...
use dirs;
use mces::headers::{MCES_HEADER_BYTES, MCES_TAG_BYTES};
use cfg_if::cfg_if;
...
const BATCH_SUBDIR: &str = "mces-batch";
...
fn vault_identity(p: &Path) -> Result<String> {
    ...
    // Read tag
    let mut tag = [0u8; MCES_TAG_BYTES];
...
// §6.11.6 Write batch rows to <USB>/mces-batch/<timestamp>.csv|.json
