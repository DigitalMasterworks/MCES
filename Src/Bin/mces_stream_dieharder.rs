//! §8.1.0 Overview — MCES v1 Cantor-Immune Stream (dieharder/practrand)
...
//!   MCES_THREADS=6 MCES_CHUNK_MB=64 cargo run --release --bin mces_stream_dieharder | \
//!       /home/luke-miller/Desktop/PractRand/RNG_test stdin32 -tlmax 40
//!
//! /* =============================================================================
//!  * MCES — mces_stream_dieharder.rs — Program v8.0.0
//!  * Numbering: Program=8.0.0, Sections=§8.X.0, Subsections=§8.X.Y
...
use mces::mces::{generate_config_with_timestamp, generate_stream, MCESConfig};
...
impl Pool {
    ...
    fn new(threads_hint: usize, cfg: Arc<MCESConfig>, postmix: Arc<Vec<u8>>) -> Self {
        ...
        for lane in 0..threads {
            ...
            let postmix_cl = Arc::clone(&postmix);
            let handle = thread::spawn(move || {
                ...
                loop {
                    ...
                    if job.len > 0 && !job.out_ptr.is_null() {
                        ...
                        if let Err(_e) = generate_stream(&cfg_cl, Some(&postmix_cl), job.offset, job.len, out_slice) {
        ...
        Self { threads, shared, handles, _cfg: cfg, _postmix: postmix }
    }
...
// ============================================================================
// §8.6.0 Main — Seed, Config, Postmix, Stream Loop
// ============================================================================
fn main() -> Result<()> {
    // §8.6.1 Unbuffered stdout (pipe-friendly)
    let mut stdout = io::stdout().lock();

    // §8.6.2 Password / seed material (supports: mces_stream_dieharder pw <password>)
    ...
    // §8.6.4 Salt = BLAKE3(ts || nonce)  (matches C)
    ...
    // §8.6.8 Argon2id OKM = k_stream_len + 32
...

    {
        ...
        let pool = Pool::new(threads_hint, Arc::clone(&cfg), Arc::clone(&postmix));
    ...
    if let Ok(mut v) = Arc::try_unwrap(postmix) {
        v.zeroize();
    }
    // (cfg is not secret; no need to wipe)
