//! §7.1.0 Overview — MCES GUI (Rust, eframe/egui)
//! File: src/bin/mces_gui.rs
//! Purpose: Desktop GUI for MCES encryption/decryption with USB sigilbook integration.
//!
//! /* =============================================================================
//!  * MCES — mces_gui.rs — Program v7.0.0
...
use mces::{
    headers::{MCES_HEADER_BYTES, MCES_TAG_BYTES, VaultHeader},
    mces::{generate_config_with_timestamp, generate_stream},
};
...
// ============================================================================
fn encrypt_file(infile: &PathBuf, user_password: Option<&str>) -> Result<String> {
    ...
    let header = VaultHeader {
        salt32, timestamp_ns, nonce12, t_cost: 3, m_cost: 17, lanes: 1, kdf_id: 2,
    };
...
fn decrypt_file(infile: &PathBuf, password: &str) -> Result<()> {
    ...
    let mut hdr = [0u8; MCES_HEADER_BYTES]; fin.read_exact(&mut hdr)?;
    let header = VaultHeader::decode(&hdr)?;
    let mut tag_file = [0u8; MCES_TAG_BYTES]; fin.read_exact(&mut tag_file)?;

    let file_size = fin.metadata()?.len();
    if file_size < (MCES_HEADER_BYTES + MCES_TAG_BYTES) as u64 {
        anyhow::bail!("invalid vault");
    }
    let clen = (file_size as usize) - (MCES_HEADER_BYTES + MCES_TAG_BYTES);
    let ct_start = (MCES_HEADER_BYTES + MCES_TAG_BYTES) as u64;

    // KDF checks + derive
    if header.kdf_id != 2 { anyhow::bail!("kdf_id unsupported"); }
...
struct App {
    ...
    batch_status: Vec<(std::path::PathBuf, &'static str)>, // [path → "OK"/"ERR"]
                    ...
                    |ui| {
                        ...
                        if enc.clicked() && !self.busy {
                            ...
                            if files.is_empty() {
                                ...
                                std::thread::spawn(move || {
                                    let res = encrypt_file(&infile, user_pw_opt.as_deref());
                                    match res {
                                        Ok(pass) => {
                                ...
                                if !self.usb_detected {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        for infile in files {
                                            ...
                                            match encrypt_file(&infile, None) {
                                                Ok(pass) => {
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native("MCES", opts, Box::new(|_cc| Box::<App>::default()))
}