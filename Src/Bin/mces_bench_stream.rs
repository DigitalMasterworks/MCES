/* =============================================================================
 * MCES — mces_bench_stream.rs — Program v4.0.0
 * Numbering: Sections §4.X.0, Subsections §4.X.Y (code-only labels)
 * Purpose: Parallel streaming keystream/ENC/DEC throughput benchmark.
 * =============================================================================
*/
...
use mces::headers::VaultHeader;
use mces::mces::{generate_config_with_timestamp, generate_stream};
...
// ============================================================================
fn main() -> Result<()> {
    /* §4.3.1 data size from env (MCES_MB, default 100 MB) */
    let mb_env = env::var("MCES_MB").ok();
    let mb: usize = mb_env
    ...
    // Split Argon2 OKM into k_stream || k_mac32
    ...
    /* §4.3.6 config + postmix (match encrypt) */
    ...
    postmix.extend_from_slice(&nonce);
    ...
    println!("[mces_bench_stream] Parallel stream: {} HW threads, data={} MB",
    ...
    if is_arm {
        println!("⚠️  [VelvetSafe Benchmark] ARM/Pi detected — timing results may be inaccurate on some hardware (clock granularity, thread pool). For best accuracy, set MCES_MB=32 and run with 1 thread.");
    }
    for &th in &sweep {
        ...
        ks_buf.par_chunks_mut(data_size/threads)
            ...
            .for_each(|(i, chunk)| {
                ...
                generate_stream(&cfg, Some(&postmix), start, chunk.len(), chunk).unwrap();
            });
        ...
        // MAC over header + length + ciphertext
        let mut mac_enc = blake3::Hasher::new_keyed(&k_mac32);
        mac_enc.update(b"MCES2DU-MAC-v1");
        ...
        mac_dec.update(b"MCES2DU-MAC-v1");
