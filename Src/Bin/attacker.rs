...
//! Simulates a co-resident “attacker” thread running while MCES encrypts.
...
//!
//! /* =============================================================================
//!  * MCES — attacker.rs — Program v9.0.0
...
use mces::mces::{generate_config_with_timestamp, encrypt_mces, MCESConfig};

// ============================================================================
// §9.3.0 Victim Role
// Purpose: perform MCES encryption on a large buffer while attacker probes.
// ============================================================================
fn victim(cfg: Arc<MCESConfig>, barrier: Arc<Barrier>, data: &[u8], ct: &mut [u8]) {
    barrier.wait(); // §9.3.1 sync start with attacker
    let _ = encrypt_mces(data, &cfg, None, ct); // §9.3.2 encrypt (mask=None for pure MCES timing)
}
...
fn main() {
    ...
    // §9.5.2 timestamp bind for MCES config
