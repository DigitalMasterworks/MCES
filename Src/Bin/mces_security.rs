// src/bin/mces_security.rs
//
// MCES Security Harness (Rust)
// ------------------------------------------------------------
// What this does:
// 1) Defines the MCES scheme components we rely on for the proof
...
//
// Notes:
// - This uses the existing Rust library in your repo (`mces::mces`) to generate
...
//
// Build & run:
//   cargo run --release --bin mces_security
//
// Optional envs:
//   MCES_SEC_TRIALS=1000  (how many trials for sanity checks; default 500)
//   MCES_SEC_MSG_BYTES=65536 (message length per trial; default 65536)
// ------------------------------------------------------------
...
fn make_postmix(k_stream: &[u8], nonce12: &[u8;12], ts_ns: u64) -> Vec<u8> {
    ...
    pm.extend_from_slice(b"MCES2DU-POST\0\0\0\0"); // exact domain sep
...
impl TrialParams {
    fn from_env() -> Self {
        let trials = std::env::var("MCES_SEC_TRIALS")
            .ok()
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(50000);
        let msg_bytes = std::env::var("MCES_SEC_MSG_BYTES")
            .ok()
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(65_536);
