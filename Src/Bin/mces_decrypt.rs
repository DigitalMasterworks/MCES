...
//!
//! /* =============================================================================
//!  * MCES — mces_decrypt.rs — Program v3.0.0
...
fn main() -> Result<()> {
    ...
    // support both: mces_decrypt <vault_file>  OR  mces_decrypt pw <password> <vault_file>
    let (password_arg, infile) = if args.len() >= 3 && args[0] == "pw" {
        (Some(args[1].clone()), args[2].clone())
    } else {
        (None, args.get(0).cloned().context("Usage: mces_decrypt <vault_file>  |  mces_decrypt pw <password> <vault_file>")?)
    };
    ...
    // §3.4.6 Derive k_stream/k_mac via Argon2id using header params
    ...
    // §3.4.9 Build MCES config + postmix
