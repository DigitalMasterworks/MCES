//! §5.1.0 Overview — MCES v1 — Cantor-Immune Stream Cipher Verdult-7 Harness (Rust port)
...
//! Output: mces_v7.log (configurable via -log)

/* =============================================================================
 * MCES — mces_v7.rs — Program v5.0.0
...
use mces::{
    mces::{generate_config_with_timestamp, generate_stream, MCESConfig},
    VAULT_VERSION,
};
...
fn parse_args() -> Params {
    let mut p = Params {
        ...
        log_path: "mces_v7.log".to_string(),
    };
...
fn build_postmix(k_stream: &[u8], ks_len: usize, nonce: &[u8; 12], ts_ns: u64) -> Vec<u8> {
    // "MCES2DU-POST\0\0\0\0" (16) || k_stream || nonce(12) || ts_be(8)
    let mut pm = Vec::with_capacity(16 + ks_len + 12 + 8);
    pm.extend_from_slice(b"MCES2DU-POST\0\0\0\0");
...
fn scheme_encrypt_vault(
    ...
    // MAC over "MCES2DU-MAC-v1" || header || len_le || ciphertext
...
fn scheme_decrypt_vault(
    ...
    if &vault[0..4] != b"MCES" { return Err(-1); }
