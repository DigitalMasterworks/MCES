...
#include <stdint.h>
#include <stdbool.h>
#include <arm_neon.h>   /* NEON for fast XOR */
...
/* ---------- NEON XOR helper (in-place) ---------- */
static inline void xor_neon_inplace(uint8_t *dst, const uint8_t *src, size_t n) {
    size_t i = 0;

    /* 64-byte blocks */
    for (; i + 64 <= n; i += 64) {
        uint8x16_t a0 = vld1q_u8(dst + i +  0), b0 = vld1q_u8(src + i +  0);
        uint8x16_t a1 = vld1q_u8(dst + i + 16), b1 = vld1q_u8(src + i + 16);
        uint8x16_t a2 = vld1q_u8(dst + i + 32), b2 = vld1q_u8(src + i + 32);
        uint8x16_t a3 = vld1q_u8(dst + i + 48), b3 = vld1q_u8(src + i + 48);
