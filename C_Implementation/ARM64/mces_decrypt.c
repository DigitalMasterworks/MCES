...
// Returns malloc'd password string (must free!), or NULL if not found
...
int main(int argc, char *argv[]) {
    ...
    if (password_arg) {
        ...
        // Unicode codepoint count/enforcement
        cp_count = 0;
        ...
        if (pw && strlen(pw) > 0) {
            ...
            if (isatty(STDIN_FILENO)) {
                ...
                if (tcgetattr(STDIN_FILENO, &oldt) == 0) {
                    ...
                    if (!fgets(password, sizeof(password), stdin)) { fprintf(stderr,"pw read fail\n"); fclose(fin); return 1; }
                }
            } else {
                if (!fgets(password, sizeof(password), stdin)) { fprintf(stderr,"pw read fail\n"); fclose(fin); return 1; }
            }
