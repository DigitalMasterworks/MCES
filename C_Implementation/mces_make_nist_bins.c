...
// mces_make_nist_bins.c  (matched to mces_stream_dieharder.c threading/KDF shape)
// Build (example):
//   gcc -O3 -DUSE_ARGON2 -o mces_make_nist_bins \
//       mces_make_nist_bins.c blake3.c mces.c -largon2 -lpthread
//
// Usage:
//   ./mces_make_nist_bins
//   NIST_FILES=2000 ./mces_make_nist_bins
//   NIST_BITS_PER_FILE=2000000 NIST_OUTDIR=out ./mces_make_nist_bins
//
// Notes:
// - Generates raw bitstreams packed LSB-first; masks high bits of last byte when bits%8 != 0.
// - Keystream offsets advance across files (no overlap).
// - Threading now uses a persistent pool clamped to (HW-2), just like mces_stream_dieharder.c.
...
/* ===== Uncapped password → config + variable postmix (match dieharder signature) ===== */
static int setup_config_and_postmix_from_password(
    const char *password,
    uint64_t timestamp_ns,
    uint8_t salt[32],           /* OUT (returned, like dieharder tool) */
    const uint8_t nonce[12],    /* IN */
    MCES_Config *cfg_out,       /* OUT */
    uint8_t **postmix_out,      /* OUT (malloc'd) */
    size_t *postmix_len_out     /* OUT */
){
    ...
    const size_t k_stream_len = round_up_32(pass_bytes);
    const size_t okm_len      = k_stream_len + 32;
...
/* ===== Parallel stream generation (persistent pool; cloned from dieharder tool) ===== */
typedef struct {
...
static void make_filename(char *buf, size_t n, const char *dir, int idx, int width) {
    if (width > 9) width = 9; /* cap width */
    snprintf(buf, n, "%s/nist_%0*u.bin", dir, width, idx);
}

/* ===== Main: write NIST bins ===== */
int main(void) {
    /* Params */
    const char *outdir = getenv("NIST_OUTDIR");
    if (!outdir || !*outdir) outdir = "nist_bins";
    ...
    /* Default NIST: 1,000,000 bits per file (≈125 KB); 1000 files ≈125 MB total */
    uint64_t bits_per_file = 1000000ull;
    const char *env_bits = getenv("NIST_BITS_PER_FILE");
    if (env_bits && *env_bits) {
        unsigned long long b = strtoull(env_bits, NULL, 10);
        if (b >= 8ull) bits_per_file = b;
    }
    int file_count = 1000;
    const char *env_files = getenv("NIST_FILES");
    if (env_files && *env_files) {
        int f = atoi(env_files);
        if (f > 0) file_count = f;
    }
    ...
    /* Config + postmix (uncapped) — return salt like dieharder tool */
    uint8_t salt[32];
    MCES_Config cfg = (MCES_Config){0};
    ...
    /* Optional: clean shutdown for the persistent pool (match dieharder) */
    atexit(gen_pool_shutdown);

    /* Keystream offsets advance across files to avoid overlap */
    uint64_t global_offset = 0;

    /* Compute bytes per file (bit-packed) */
    uint64_t bytes_per_file = (bits_per_file + 7ull) / 8ull;
    if (bytes_per_file == 0) { fprintf(stderr,"bytes_per_file=0\n"); goto bye; }

    /* Make chunk reasonable relative to bytes_per_file */
    if (chunk > bytes_per_file) chunk = (size_t)bytes_per_file;

    /* Temp buffer */
    uint8_t *buf = (uint8_t*)malloc(chunk ? chunk : 1);
    if (!buf) { fprintf(stderr,"OOM\n"); goto bye; }

    /* File index width for names */
    int width = 1;
    for (int t = file_count; t >= 10; t /= 10) width++;

    fprintf(stderr, "[mces_nist] outdir=%s files=%d bits_per_file=%llu (~%llu bytes)\n",
            outdir, file_count, (unsigned long long)bits_per_file, (unsigned long long)bytes_per_file);
    fprintf(stderr, "[mces_nist] threads_hint=%d (pool clamps to HW-2) chunk=%zu bytes\n", threads_hint, chunk);

    for (int i = 1; i <= file_count; ++i) {
        char path[1024];
        make_filename(path, sizeof(path), outdir, i, width);
        ...
        uint64_t remaining = bytes_per_file;
        while (remaining) {
            size_t want = (remaining < (uint64_t)chunk) ? (size_t)remaining : chunk;

            if (gen_stream_parallel(&cfg, postmix, postmix_len, global_offset, want, buf, threads_hint) != 0) {
                fprintf(stderr,"gen_stream_parallel failed (file %d)\n", i);
                fclose(f);
                goto loop_break;
            }

            size_t w = fwrite(buf, 1, want, f);
            if (w != want) {
        ...
        /* Mask the final byte’s high bits if bits_per_file not multiple of 8 */
        if ((bits_per_file & 7ull) != 0) {
            long pos = ftell(f);
            if (pos > 0) {
                if (fseek(f, -1L, SEEK_CUR) == 0) {
                    int c = fgetc(f); /* read last byte */
                    if (c != EOF) {
                        uint8_t last = (uint8_t)c;
                        int valid = (int)(bits_per_file & 7ull);
        ...
        fclose(f);
        if ((i % 50) == 0 || i == file_count) {
            fprintf(stderr, "[mces_nist] wrote %d/%d: %s\n", i, file_count, path);
        }
    }
