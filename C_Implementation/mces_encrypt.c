...
size_t utf8_codepoints(const char *s, uint32_t *indices) {
    size_t count = 0;
    for (size_t i = 0; s[i]; ++i) {
        if ((s[i] & 0xC0) != 0x80) {
            if (indices) indices[count] = (uint32_t)i;
            count++;
        }
    }
    return count;
...
int try_save_to_sigilbook(const char *vault_path, const char *password) {
    int status = 1;
    pid_t pid = fork();
    if (pid == 0) {
...
int main(int argc, char *argv[]) {
    ...
    if (password) {
        ...
        cp_count = utf8_codepoints(password, indices);
        free(indices);
        ...
        // --- Random Unicode password mode (your original logic) ---
